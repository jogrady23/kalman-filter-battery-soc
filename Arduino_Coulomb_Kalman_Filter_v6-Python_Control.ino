 ...
 * DESCRIPTION:
 * ------------
 * This program calculates the number of Coulombs leaving or entering a battery.
 * This is accomplished by measuring the current of a circuit over discrete time intervals, 
 * then mutliplying the current by the time interval to get the number of Coulombs. 
 * The program is intended to be run on an Arduino/Genuino Uno and uses the analog pin A0. Additionally,
 * the circuit uses a 1.3 +/- 0.1 Ohm power resistor, with the analog pin reading the voltage difference
 * between both ends of the power resistor (by setting the second end to GND on the Arduino)
 * 
 * CONTACT:
 * -----------
 * Jack O'Grady
